...
@section('title', 'Détails de l\'Avis - Réclamation #{{ $reclamation->id }}')
...
<div class="content-wrapper">
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Avis #{{ $avis->id }} pour "{{ $reclamation->sujet }}"</h4>
                    ...
                    <div class="form-group mt-4">
                        <a href="{{ route('admin.reclamations.avis.edit', ['reclamation' => $reclamation->id, 'avis' => $avis->id]) }}" class="btn btn-warning">
                            <i class="ti-pencil"></i> Modifier
                        </a>
                        <a href="{{ route('admin.reclamations.avis.index', $reclamation->id) }}" class="btn btn-secondary">
                            <i class="ti-arrow-left"></i> Retour aux Avis
                        </a>
                        <form action="{{ route('admin.reclamations.avis.destroy', ['reclamation' => $reclamation->id, 'avis' => $avis->id]) }}" method="POST" style="display: inline;" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cet avis ?')">
                            @csrf
                            @method('DELETE')
